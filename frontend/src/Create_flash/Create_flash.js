...
const Create_flash = () => {
  ...
  useEffect(() => {
    ...
    dropdownStatus ? tl.play() : tl.reverse();
  }, [dropdownStatus]);

  const handleDropdownClick = () => setDropdownStatus(!dropdownStatus);

  const handleSelectClick = () => setCustomSelectActive(!customSelectActive);

  const handleOptionClick = (e) => {
    setSelectedValue(e.currentTarget.querySelector("label").textContent);
    setCustomSelectActive(false);
  };

  const handleOptionClickBrown = (e) => {
    setSelectedValueBrown(e.currentTarget.querySelector("label").textContent);
    setCustomSelectActive(false);
  };

  const handlePopupClick = () => setPopupStatus(true);
  const handleClosePopup = () => setPopupStatus(false);
          ...
          <div className="navigation__dropdown">
            ...
            <ul className="navigation__dropdown-list">
              <div className="navigation__dropdown-button-container">
                {["Flash-slices", "Quick-bites"].map((text, i) => (
                  <button
                    key={i}
                    className={`navigation__dropdown-button navigation__dropdown-button-${
                      i + 1
                    }`}
                  >
                    {text}
